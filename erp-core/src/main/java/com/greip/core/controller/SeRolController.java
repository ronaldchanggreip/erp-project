...
import com.greip.core.service.SeRolService;
...
public class SeRolController extends BaseController{

    public static Logger log = Logger.getLogger(SeRolController.class);

    @Autowired
    SeRolService service;
    ...
    public ResponseEntity<GeMensajeControllerDto> gets(@RequestParam("filtro") String filtro) {
        ...
        try {
            ...
            lista = (List<SeRolDto>) this.service.gets(filtroDto);
            if (lista.isEmpty()) {
    ...
    @CrossOrigin(origins = "*")
    public ResponseEntity<GeMensajeControllerDto> save(@RequestBody SeRolDto dto, OAuth2Authentication auth) {
        HttpStatus httpStatus = null;
        ...
        if (dto == null) {
            ...
            if(dto.getId()!=null){
                ...
                try {
                    ...
                    httpStatus = HttpStatus.OK;
                    respuesta = UtlController.getMesajeController(HttpStatus.OK.value());
                    respuesta.setMensajeUsuario("El Rol se guardó correctamente!");
                    respuesta.setRespuesta(dto);
    ...
    public ResponseEntity<GeMensajeControllerDto> save(@PathVariable("id") BigInteger id, @RequestBody SeRolDto dto, OAuth2Authentication auth) {
        HttpStatus httpStatus = null;
    ...
    public ResponseEntity<GeMensajeControllerDto> eliminarLogico(@RequestBody List<BigInteger> lst, OAuth2Authentication auth) {
        ...
        if (lst == null) {
            ...
            try {
                ...
                httpStatus = HttpStatus.OK;
                respuesta = UtlController.getMesajeController(HttpStatus.OK.value());
                respuesta.setMensajeUsuario("El rol ha sido eliminado!");
            } catch (ServiceException e) {
                httpStatus = HttpStatus.INTERNAL_SERVER_ERROR;
                respuesta = UtlController.getMesajeController(UtlController.codErrorService);
                respuesta.setDetalle(e.getMessage());
