    ...
    import com.greip.core.service.GeGrupoParametroService;
...
public class GeGrupoParametroController extends BaseController{

    public static Logger log = Logger.getLogger(GeGrupoParametroController.class);

    @Autowired
    GeGrupoParametroService service;
    ...
    public ResponseEntity<GeMensajeControllerDto> gets(@RequestParam("filtro") String filtro) {
        ...
        try {
            ...
            lista = (List<GeGrupoParametroDto>) this.service.gets(filtroDto);
            if (lista.isEmpty()) {
    ...
    @CrossOrigin(origins = "*")
    public ResponseEntity<GeMensajeControllerDto> save(@RequestBody GeGrupoParametroDto dto, OAuth2Authentication auth) {
        HttpStatus httpStatus = null;
        ...
        if (dto == null) {
            ...
            if(dto.getId()!=null){
                ...
                try {
                    ...
                    httpStatus = HttpStatus.OK;
                    respuesta = UtlController.getMesajeController(HttpStatus.OK.value());
                    respuesta.setMensajeUsuario("El Grupo Parámetro se guardó correctamente!");
                    respuesta.setRespuesta(dto);
    ...
    @CrossOrigin(origins = "*")
    public ResponseEntity<GeMensajeControllerDto> save(@PathVariable("id") BigInteger id, @RequestBody GeGrupoParametroDto dto) {
        HttpStatus httpStatus = null;
